<nav>
    <div class="nav nav-tabs size-tab" id="nav-tab" role="tablist">
        <button class="nav-link {{ session('finalPriceData.shape') == 'Square' ? 'active' : '' }}" id="tabSize-first" data-bs-toggle="tab" data-bs-target="#size-first" type="button" role="tab" aria-controls="nav-home" aria-selected="true">Recomended</button>
...
<div class="tab-content mt-2" id="nav-tabContent">
    <div class="tab-pane fade {{ session('finalPriceData.shape') == 'Square' ? 'active show' : '' }}" id="size-first" role="tabpanel" aria-labelledby="tabSize-first">
        <div class="paddWrapper">
            <div class="radio-group row">
                @foreach ($recommended_data as $key => $value)
                    <div class="col-md-3 col-4">
                        <label class="custom-radio size {{ $finalPriceData['size'] == $value['name']  ? 'active' : '' }}">
                            <input {{ $finalPriceData['size'] == $finalPriceData['size']  ? 'checked' : '' }} type="radio" name="size" value="{{ $key }}" data-type="Recommended" data-name="{{ $value['name'] }}" data-price="{{ $value['price'] }}" class="frame-option" >
            ...
            <div class="radio-group row">
                @if($square_data)
                    @foreach ($square_data as $key => $value)
                        <div class="col-md-3 col-4">     
                            <label class="custom-radio size {{ $finalPriceData['size'] == $value['name']  ? 'active' : '' }}" >
                                <input {{ $finalPriceData['size'] == $value['name']  ? 'checked' : '' }} type="radio" name="size" value="{{ $key }}" data-type="Square" data-name="{{ $value['name'] }}" data-price="{{ $value['price'] }}" class="frame-option" > 
                                <div class="object" style="height:{{ $value['height'] }}px; width:{{ $value['width'] }}px;"></div>
